-- - - -- --- ----- -------- ------------- ---------------------
-- Rename table "hotel" to "location".
-- Update "conference" references as well.
--
-- Done via helper tables as sqlite's ALTER TABLE can only append new
-- columns and rename entire tables

ALTER TABLE hotel RENAME TO location
;
...
SET value = (SELECT value
	     FROM config
	     WHERE key = '@current-hotel')
WHERE key = '@current-location'
;
