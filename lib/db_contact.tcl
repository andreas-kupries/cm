...
namespace eval ::cm::db::contact {
	...
	\
	select label known known-email issues \
	setup dump
    namespace ensemble create
...
proc ::cm::db::contact::relations-formatted {contact type} {
    debug.cm/db/contact {}
...
proc ::cm::db::contact::emails-limited {mails} {
    debug.cm/db/contact {}
    setup

    return [db do eval [string map [list @@ [join $mails ,]] {
	SELECT id, email
	FROM   email
	WHERE  id IN (@@)
	ORDER BY email
